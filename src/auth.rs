...
impl Auth {
  ...
  pub fn generate_key_pair(public_key_path: &str, private_key_path: &str) -> Result<()> {
    ...
    std::fs::write(private_key_path, signing_key.to_bytes())
      .map_err(|e| Box::new(AdeployError::FileSystem(format!("Failed to write private key: {}", e))))?;
    ...
    std::fs::write(public_key_path, public_key_str)
      .map_err(|e| Box::new(AdeployError::FileSystem(format!("Failed to write public key: {}", e))))?;
    
    info!("Generated Ed25519 key pair at: {} and {}", private_key_path, public_key_path);
    Ok(())
  }
  ...
  pub fn load_key_pair(private_key_path: &str) -> Result<SigningKey> {
    ...
    let private_key_bytes = std::fs::read(private_key_path)
      .map_err(|e| Box::new(AdeployError::FileSystem(format!("Failed to read private key: {}", e))))?;
    
    if private_key_bytes.len() != 32 {
      return Err(Box::new(AdeployError::Auth("Invalid private key length".to_string())));
    }
    
    let signing_key = SigningKey::from_bytes(&private_key_bytes.try_into()
      .map_err(|_| Box::new(AdeployError::Auth("Failed to convert private key bytes".to_string())))?);
  ...
  pub fn load_public_key<P: AsRef<Path>>(path: P) -> Result<String> {
    std::fs::read_to_string(path)
      .map_err(|e| Box::new(AdeployError::FileSystem(format!("Failed to read public key: {}", e))))
  }
  ...
  pub fn sign_data(&self, data: &[u8]) -> Result<Vec<u8>> {
    if let Some(signing_key) = &self.keypair {
      ...
      Err(Box::new(AdeployError::Auth("No keypair available for signing".to_string())))
    }
  }

  /// Verify Ed25519 signature
  pub fn verify_signature(public_key_str: &str, data: &[u8], signature_bytes: &[u8]) -> Result<bool> {
    // Decode the base64 public key directly
    let public_key_bytes = base64::engine::general_purpose::STANDARD.decode(public_key_str.trim())
    ...
    let verifying_key = VerifyingKey::from_bytes(&public_key_bytes.try_into()
      .map_err(|_| Box::new(AdeployError::Auth("Failed to convert public key bytes".to_string())))?)
      .map_err(|e| Box::new(AdeployError::Auth(format!("Failed to parse public key: {}", e))))?;
    
    // Create signature object
    let signature = Signature::from_bytes(signature_bytes.try_into()
      .map_err(|_| Box::new(AdeployError::Auth("Failed to convert signature bytes".to_string())))?);
